...
1. Choose _Galasa > Setup Galasa Workspace_ from the main Eclipse menu - this command creates some necessary configuration files. Your Eclipse console confirms its progress with some messages:
   ...
   Setup complete
   ```
Note: You can ignore the message _Not creating a ~/.m2/settings.xml as the Galasa remote maven uri preference is the default location of <div>https://repo.maven.apache.org/maven2/</div>_ if it appears in the logs.
